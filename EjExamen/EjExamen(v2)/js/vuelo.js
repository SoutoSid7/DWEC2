...
// ⭐ LOCALSTORAGE: Cargar datos desde localStorage al iniciar
function cargarDesdeStorage() {
    // Busca en el localStorage la clave vuelos y vuelosMuyRentables
    const vuelosGuardados = JSON.parse(localStorage.getItem("vuelos")) || []
    const rentablesGuardados = JSON.parse(localStorage.getItem("vuelosMuyRentables")) || []

    // Recorre el array vuelosGuardados, crea un objeto de la clase Vuelo con esos datos
    vuelosGuardados.forEach(v => {
        vuelos.push(new Vuelo(v.codigo, v.numPlazas, v.imporB))
    })

    rentablesGuardados.forEach(v => {
        vueloMuyRentable.push(new VueloMuyRentable(v.codigo, v.ingreso))
    })
...
// ⭐ LOCALSTORAGE: Guardar datos tras cada cambio
function guardarEnStorage() {
    localStorage.setItem("vuelos", JSON.stringify(vuelos))
    localStorage.setItem("vuelosMuyRentables", JSON.stringify(vueloMuyRentable))
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    cargarDesdeStorage() // ⭐ LOCALSTORAGE
